...
ele_main.addEventListener("mousedown", (ev) => {
        var oEvent = ev || event;
        if (oEvent.button == 0) {
            ml = 1;
            mm = 0;
...
ele_main.addEventListener("mousewheel", (ev) => {
        ev.preventDefault();
        var oEvent = ev || event;
        len *= Math.exp(-0.001 * oEvent.wheelDelta);
    },false);
ele_main.addEventListener("touchstart", (ev) => {
...
window.onresize = function () {
    ...
    ele_main.style.width = 1024 + "px";
    ele_main.style.height = 1024 + "px";
    ele_main.style.transform = "scale(" + cx / 1024 + "," + cy / 1024 + ")";
}
cx = document.body.clientWidth;
cy = document.body.clientHeight;
if (cx > cy) {
